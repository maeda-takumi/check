...
<main class="container">
  <section class="card">
    ...
    <?php foreach ($groups as $group): ?>
      <hr>
      <h3><?= htmlspecialchars((string)$group['group_name'], ENT_QUOTES, 'UTF-8'); ?></h3>
      <form method="post" class="inline-form">
        <input type="hidden" name="action" value="toggle_group">
        <input type="hidden" name="group_id" value="<?= (int)$group['group_id']; ?>">
        <input type="hidden" name="is_active" value="<?= (int)$group['is_active']; ?>">
        <button type="submit"><?= (int)$group['is_active'] === 1 ? 'グループ無効化' : 'グループ有効化'; ?></button>
      </form>
      ...
      <?php if (!empty($questionsByGroup[(int)$group['group_id']])): ?>
        <ul class="list">
          <?php foreach ($questionsByGroup[(int)$group['group_id']] as $question): ?>
            <li>
              <strong><?= htmlspecialchars((string)$question['question'], ENT_QUOTES, 'UTF-8'); ?></strong>
              <small>(<?= (int)$question['check_type'] === 1 ? 'checkbox' : 'text'; ?><?= (int)$question['required'] === 1 ? '/必須' : ''; ?>)</small><br>
              <form method="post" class="inline-form">
